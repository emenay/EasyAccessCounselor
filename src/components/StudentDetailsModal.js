...
function CollegeListPanel(props) {
  ...
  const refreshWithDatabase = () => {
    ...
    if (props.studentinfo.collegeList) {
      let reachCheap = props.studentinfo.collegeList.Reach.filter(
        (p) => p.Affordability === 1
      ).map((p) => (
        <div>
          <text class="fieldElement">{p.SchoolName}</text>
        </div>
      ))
      let reachMid = props.studentinfo.collegeList.Reach.filter(
        (p) => p.Affordability === 2
      ).map((p) => (
        <div>
          <text class="fieldElement">{p.SchoolName}</text>
      ...
      let reachCostly = props.studentinfo.collegeList.Reach.filter(
        (p) => p.Affordability === 3
      ).map((p) => (
        <div>
          <text class="fieldElement">{p.SchoolName}</text>
      ...
      let targetCheap = props.studentinfo.collegeList.Target.filter(
        (p) => p.Affordability === 1
      ).map((p) => (
        <div>
          <text class="fieldElement">{p.SchoolName}</text>
      ...
      let targetMid = props.studentinfo.collegeList.Target.filter(
        (p) => p.Affordability === 2
      ).map((p) => (
        <div>
          <text class="fieldElement">{p.SchoolName}</text>
      ...
      let targetCostly = props.studentinfo.collegeList.Target.filter(
        (p) => p.Affordability === 3
      ).map((p) => (
        <div>
          <text class="fieldElement">{p.SchoolName}</text>
      ...
      let safetyCheap = props.studentinfo.collegeList.Safety.filter(
        (p) => p.Affordability === 1
      ).map((p) => (
        <div>
          <text class="fieldElement">{p.SchoolName}</text>
      ...
      let safetyMid = props.studentinfo.collegeList.Safety.filter(
        (p) => p.Affordability === 2
      ).map((p) => (
        <div>
          <text class="fieldElement">{p.SchoolName}</text>
      ...
      let safetyCostly = props.studentinfo.collegeList.Safety.filter(
        (p) => p.Affordability === 3
      ).map((p) => (
        <div>
          <text class="fieldElement">{p.SchoolName}</text>
            ...
            for (let i = 0; i < colleges.length; i++) {
              ...
              let colElement = (
                <CollegeElement
                  ...
                  pub={pubOrPriv}
                  needmet={
                    colleges[i].needmet !== 'NA'
                      ? colleges[i].needmet * 100 + '% Need Met'
                      : ''
                  }
  ...
  return (
    <div>
      ...
      <table class="colListTable3">
        <tbody>
          <tr>
            <th style={z}>Search for Colleges</th>
          </tr>
        </tbody>
        <tbody>
          <tr>
            <td style={z}>
              <div>Filter Using Counselor Info</div>
              <button
                onClick={(e, searchtype) => searchCollege(e, 'filter')}
          ...
          <tr>
            <td>
              <span>
                {validate(props) && (
                  <button
                    class="colListButton bigbutton"
                    onClick={() => categorizeColleges(props, state.checkedIds)}>
                    Add to College List
                  </button>
                )}
                {!validate(props) && (
                  <Popup
                    trigger={
                      <button class="colListButton bigbutton">
                        Add to College List
                      </button>
                    }
                    modal>
                ...
                <button
                  class="colListButton mediumbutton"
                  onClick={() => removeColleges()}
                  className="colListButton mediumbutton">
                  Remove
                </button>
              </span>
            </td>
          ...
          <tr id="collegesearchresults">
            <td>
              <table class="resultsTable" style={{ width: '90%' }}>
                <tbody class="results">{searchResults}</tbody>
              </table>
            </td>
          </tr>
        </tbody>
  ...
  return (
    <div className="caseload-panel">
      <div className="caseload-meetingsnum">
        ...
        <p>
          {'Total - ' +
            (meetingsNumber(info, 'individualMeetings') +
              meetingsNumber(info, 'eventMeetings') +
              meetingsNumber(info, 'groupMeetings'))}
        </p>
      </div>
      <div className="caseload-maincontent">
        ...
        <div className="caseload-noteitem">
          <input
            type="date"
            className="caseload-enterdate"
            value={date}
            onChange={(e) => changeDate(e.target.value)}
          />
          <select
            value={noteType}
            className="caseload-notetype"
          ...
          <div>
            <textarea
              className="caseload-notetext"
              value={text}
              onChange={(e) => changeText(e.target.value)}
            />
            <button
              className="caseload-savebutton"
              onClick={() => {
                uploadNote(
                  date,
                  noteType,
        ...
        {notes.map((note, index) => {
          return (
            <div className="caseload-noteitem" key={index}>
              <p className="caseload-textitem">
                {typeof note.date === 'string'
                  ? note.date
                  : note.date.toDate().toISOString().slice(0, 10)}
              </p>
              <p className="caseload-textitem">{note.type}</p>
              <p
                className={
                  'caseload-textitem itemhoverable' +
                  (noteViewed === note ? ' viewingNote' : '')
                }
                onClick={() => {
                  noteViewed === note
                    ? changeNoteViewed(null)
                    : changeNoteViewed(note)
                }}>
                {note.text}
              </p>
            </div>
