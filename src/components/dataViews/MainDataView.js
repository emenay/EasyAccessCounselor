...
function changeFilter(filterSettings) {
  ...
  var newData = arr[0].filter(function (el) {
    ...
    if (
      filterSettings['Goalfilter'] !== undefined &&
      filterSettings['Goalfilter'].length > 0
    ) {
    ...
    if (
      filterSettings['Early Appsfilter'] !== undefined &&
      filterSettings['Early Appsfilter'].length > 0
    ) {
    ...
    if (
      filterSettings['Essayfilter'] !== undefined &&
      filterSettings['Essayfilter'].length > 0
    ) {
    ...
    if (
      filterSettings['Resultsfilter'] !== undefined &&
      filterSettings['Resultsfilter'].length > 0
    ) {
    ...
    if (
      filterSettings['Decisionfilter'] !== undefined &&
      filterSettings['Decisionfilter'].length > 0
    ) {
  ...
  handleClick = (event) => {
    ...
    if (event.target.className.includes('is-active')) {
      if (this.state[event.target.name + 'filter'] != undefined) {
        arr = arrayRemove(
          this.state[event.target.name + 'filter'],
          event.target.id
        )
        ...
        {Object.entries(this.state.uniqueValues).map(([name, info]) => (
          <div
            className={
              ...
              (this.state[name + 'collapse'] ? '' : ' is-active')
            }
            tabIndex="0"
            key={name}
...
class MainDataView extends React.Component {
  ...
  handleClick = (event) => {
    ...
    navNodes.forEach((e) => {
      if (e.className.includes('is-active')) e.classList.remove('is-active')
    })
    event.target.parentElement.classList.add('is-active')

    switch (event.target.id) {
      case 'grid_view':
        currentComponent.setState({ view: 'grid_view' })
        break
      case 'block_view':
        currentComponent.setState({ view: 'block_view' })
        break
      case 'table_view':
        currentComponent.setState({ view: 'table_view' })
        break
      default:
        console.log('here')
    }
  ...
  render() {
    return (
      <div>
        <div>
          <div className="field has-addons">{inputSearch([])}</div>
          <div className="tabs is-centered" id="tabs">
            <ul id="navigation">
              <li className="btn is-active" id="tview">
                <a id="table_view" className="navbar-item tab">
                  Table View
                </a>
              </li>
            </ul>
